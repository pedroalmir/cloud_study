 ...
 * @author Pedro Almir
 */
@WebServlet(name = "LikePostController", urlPatterns = { "/post/like" })
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
	    	...
	    	if(email != null && !email.isEmpty() && postID != null && isLike != null) {
	    		...
	    		if(post != null) {
	    			MyCloudDatastoreService datastoreService = new MyCloudDatastoreService();
	    			if(isLike) {
	    				datastoreService.likePost(post, email);
	    			}else {
	    				datastoreService.unlikePost(post, email);
	    			}
